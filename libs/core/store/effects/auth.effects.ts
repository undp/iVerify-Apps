...
export class AuthEffects {
    ...
    logout: Observable<any> = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(() => {
                ...
                return [new LogoutSuccess(), new ClearUserPermissions()];
            })
        )
    );

    getCurrentUser: Observable<any> = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(() => {
                return this.authService.me().pipe(
                    switchMap((user) => [
                        user
                            ? new GetCurrentUserSuccess(user.data)
                            : new GetCurrentUserFailure(),
                    ]),
    ...
    getUserPermissions: Observable<any> = createEffect(() =>
        combineLatest(
            this.actions$.pipe(
                ofType<GetCurrentUserSuccess>(
                    EAuthActions.GetCurrentUserSuccess
                )
            ),
            this.actions$.pipe(
                ofType<GetPermissionsSuccess>(
                    EAuthActions.GetPermissionsSuccess
                )
            ),
            this.actions$.pipe(
                ofType<GetRolesSuccess>(EAuthActions.GetRolesSuccess)
            )
        ).pipe(
            switchMap(
                ([getCurrentUserSuccess, getPermission, getRolesSuccess]) => [
