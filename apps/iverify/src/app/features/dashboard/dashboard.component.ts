...
import {
    TicketRequest,
    StatusFormat,
    StatusFormatPieChart,
    TicketsByAgentFormat,
    BubbleChartFormat,
    ChartTypeEnum,
} from '@iverify/core/models/dashboard';
...
export class DashboardComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    ngOnInit() {
        let previousMonday = new Date(
            DashboardHelpers.GetPreviousWeekFirstDay()
        );
            ...
            this.dashboardService
                ...
                .subscribe((res) => {
                    ...
                    this.statsByCategories =
                        DashboardHelpers.SortStatistics(res);
                    this.ticketsByChannel =
                        DashboardHelpers.GetTicketsByChannel(
                            this.statsByCategories[CountBy.source]
                        );
                    this.ticketsByTag = DashboardHelpers.GetTicketsByTag(
                        this.statsByCategories[CountBy.tag]
                    );
                    this.ticketsByType = DashboardHelpers.GetTicketsByType(
                        this.statsByCategories[CountBy.violationType]
                    );
                    this.ticketsByWeek = DashboardHelpers.GetTicketsByFolder(
                        this.statsByCategories[CountBy.verifiedByDay],
                        CountBy.folder
                    ); //DashboardHelpers.GetTicketsByWeek(res, this.options);
                    this.ticketsByAgents = DashboardHelpers.GetTicketsByAgents(
                        this.statsByCategories
                    );
                    this.ticketsByFolder = DashboardHelpers.GetTicketsByFolder(
                        this.statsByCategories[CountBy.folder],
                        CountBy.folder
                    );
                    this.ticketsByToxicity =
                        DashboardHelpers.GetTicketsByFolder(
                            this.statsByCategories[CountBy.toxicity],
                            CountBy.toxicity
                        );
                    this.ticketsReponseTime =
                        DashboardHelpers.GetTicketsReponseTime(
                            this.statsByCategories[CountBy.responseVelocity],
                            this.title
                        );
                    if (this.isDefaultData) {
                        this.ticketsByCurrentStatus =
                            DashboardHelpers.GetTicketsByCurrentStatus(
                                this.statsByCategories
                            );
                        this.totalPublished =
                            this.ticketsByCurrentStatus &&
                            this.ticketsByCurrentStatus[2]
                                ? this.ticketsByCurrentStatus[2].value
                                : 0;
                    }
                })
        );
    ...
    getAllTicketsData() {
        if (this.selectedTimeType === 1) {
            this.ticketsReponseTime = DashboardHelpers.GetTicketsReponseTime(
                this.statsByCategories[CountBy.responseVelocity],
                this.title
            );
            ...
            this.ticketsReponseTime = DashboardHelpers.GetTicketsReponseTime(
                this.statsByCategories[CountBy.resolutionVelocity],
                this.title
            );
