...
import {
    ...
    IndicatorDetailState,
} from './indicator-detail.service';
...
export class IndicatorDetailComponent {
    ...
    chartTypeValues = EnumValues.getValues(ChartTypeEnum).filter(
        (v) => v !== ChartTypeEnum.BUBBLE.toString()
    );
    ...
    chartTypeChanges$: Observable<any> =
        this.form.controls['chartSelection'].valueChanges;
    chartTypeValues$: Observable<any> = this.dataType$.pipe(
        filter((t) => !!t),
        map((type: CountBy) => {
            console.log('datatypeeeeee: ', type);
            if (type === CountBy.agentAllStatuses || type === CountBy.toxicity)
                return this.chartTypeValues.filter((v) =>
                    this.chartsForAgentsAllStatuses.includes(v.toString())
                );
    ...
    constructor(
        ...
        this.chartTypeChanges$.subscribe((value) =>
            this.stateService.updateChartType(value)
        );
    }
