...
export class UsersController {
    ...
    async editUser(
        ...
        const editedUser = await this.usersService.update(
            user.userId,
            editUserDto
        );
        if (!editedUser)
            throw new BadGatewayException(userMessages.userUpdateFail);
    ...
    async deleteUser(@Query() user: GetUserDto) {
        ...
        if (!deletedUser)
            throw new BadGatewayException(userMessages.userDeleteFail);
