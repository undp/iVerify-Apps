...
export class RolesGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (!reqMethod || !reqUrl || !userData.id)
            throw new BadRequestException();
        if (userData && userData.roles) {
            const Roledata = userData.roles[0];
            const rolePermissionStatus = await this.roleHasPersmission(
                Roledata,
                reqUrl,
                reqMethod
            );
    ...
    async roleHasPersmission(
        roleData,
        resource: string,
        method: string
    ): Promise<boolean> {
        const resources =
            roleData && roleData.resource ? roleData.resource : null;
        ...
        const resourcePermission = resourcesData.find(
            (o) =>
                o && o.name && o.name.toLowerCase() === resource.toLowerCase()
        );

        if (!(resourcePermission && resourcePermission.permissions)) {
            return false;
        }
