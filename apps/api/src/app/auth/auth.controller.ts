import { Controller, UseGuards, HttpStatus, Response, Post, Body, UseFilters, BadRequestException, NotFoundException, BadGatewayException } from '@nestjs/common';
...
export class AuthController {
    ...
    constructor(private readonly authService: AuthService,
        private readonly usersService: UsersService, private InfoLogger: InfoLogger) {
    ...
    public async login(@Body() login: LoginUserDto) {
        const userData = await this.usersService.findOne({ email: login.email });
        if (!userData) {
    ...
    public async generateToken(@Body() userData: TokenGenerationDto) {
        return await this.usersService.findOne({ email: userData.email }).then(user => {
            if (!user) {
                this.InfoLogger.error(userMessages.userNotFound);
